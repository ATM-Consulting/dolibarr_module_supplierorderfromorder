...
$hookmanager->initHooks(array('supplierrderlist', 'globalcard')); // Note that conf->hooks_modules contains array
...
if(strpos($sortfield, 'cf.') === false
	...
	&& strpos($sortfield, 'typent.') === false
	&& strpos($sortfield, 'state.') === false
	&& strpos($sortfield, 'p.') === false
	&& strpos($sortfield, 'u.') === false) {
	...
	<script type="text/javascript" language="javascript">
		$(document).ready(function() {
			$.ajax({
				...
				// Formulaire
				form_commandes.attr('action', form_commandes.attr('action').replace("<?php print dol_buildpath('/fourn/commande/list.php', 1); ?>", "<?php print dol_buildpath('/supplierorderfromorder/ordercustomer.php', 1); ?>"));
				form_commandes.attr('action', form_commandes.attr('action') + '?id=' + <?php print $id; ?>);
